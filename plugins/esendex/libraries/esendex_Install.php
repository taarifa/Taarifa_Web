...
class Esendex_Install {
	...
	public function run_install()
	{
		...
		// Settings Table
		$this->db->query("
			CREATE TABLE IF NOT EXISTS `".Kohana::config('database.default.table_prefix')."esendex_settings`
			(
				id int(11) unsigned NOT NULL AUTO_INCREMENT,
				esendex_key varchar(100) DEFAULT NULL,
		...
		$this->db->query("
			...
			(
				...
				esendex_type varchar(20) DEFUALT NULL COMMENT 'SMS Type Field',
				esendex_indexid varchar(15) NOT NULL COMMMENT 'esendex IndexID -used for requesting messages after),
				PRIMARY KEY (`myid`)
			);
			COMMENT ON TABLE 
		");
	...
	{
		$this->db->query("
			DROP TABLE ".Kohana::config('database.default.table_prefix')."esendex_settings;
